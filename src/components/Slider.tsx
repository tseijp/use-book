...
export function Slider ({children, width=600, visible=4}:SliderProps) {
    ...
    const [springs, set] = useSprings(children.length, i => ({ x : (i<children.length ?i: 0)*width }) )
  //const [springs, set] = useSprings(children.length, i => ({ x : (i<children.length-1?i:-1)*width }) )
    ...
    return null
    return (
        <div {...bind()} style={{position:"fixed",width:"100%",height:"100%"}}>
          {springs.map(({x}, i) => (
            <animated.div key={i} style={{position:'absolute',x,width,height:'50%',willChange:'transform',bottom:0,left:0}}>
                {children[i]}
            </animated.div>
          ))}
        </div>
